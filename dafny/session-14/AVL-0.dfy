...
{    
   if abs(myHeight(l)-myHeight(r)) <= 1 then
      node(l, x, r)
   else if myHeight(l) == myHeight(r) + 2 then
      leftUnbalance(l, x, r)
   else
      rightUnbalance(l, x, r)
}
...
ensures max(height(l), height(r)) <= height(res) <= max(height(l), height(r)) + 1		
{
  if myHeight(l.left)  >= myHeight(l.right) 
     then // this is the LL rotation
          heights(l.left); heights(l.right);
          assert height(l.right) <= height(l.left);
          ...
          assert tset(l.right.right) < tset(l);
          assert promising(l.left, l.key, l.right.left);
          assert promising(l.right.right, x, r);
          heights(l.left); heights(l.right);
