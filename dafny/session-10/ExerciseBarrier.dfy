...
requires v.Length > 0
requires 0 <= p < v.Length
ensures b == (forall i:int, j:int | 0 <= i <= p < j < v.Length :: v[i] < v[j])
{
...
while (i < v.Length)
    ...
    invariant forall k | 0 <= k < i <= p + 1 :: maxl <= v[k];
    invariant b == (forall k | p < k < i <= v.Length :: v[k] > maxl);
{
    if (i <= p) {
        if (v[i] < maxl) {
            maxl := v[i];
