...
\begin{code}
{-@ type ListUn a X Y = ListS a {union (elts X) (elts Y)} @-}
\end{code}

+ A list whose contents are exactly `X` and the contents of `Y`

\begin{code}
{-@ type ListUn1 a X Y = ListS a {union (singleton X) (elts Y)} @-}
\end{code}
...
reverse         = go []
  where
    {-@ go     :: acc:[a] -> xs:[a] -> [a] @-}
--    {-@ go     :: acc:UList a -> xs:{v:UList a | Disjoint (elts acc) (elts v)} 
--                 -> UList a @-}  
--    {-@ go     :: acc:UList a -> xs:{v:UList a | Disjoint (elts acc) (elts v)} 
--                  -> {v:UList a| elts v == Set_cup (elts acc) (elts xs)} @-}
    go acc []     = acc
...
--{-@ append       :: xs:UList a -> ys:{v:UList a | Disjoint (elts xs) (elts v)} 
