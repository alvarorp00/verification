...
reverse xs        = go [] xs
  where
-- added signature
{-@ go :: xs:List a -> ys:List a ->  {v:List a | size v == size xs + size ys} @-}
    go acc []     = acc
...
\begin{code}
-- added signature
{-@ zipOrNull :: xs:List a -> ys:{v:List b | 
        size xs > 0 && size v > 0 ==> size v == size xs}
    -> {v:List (a,b) | (size xs == 0 || size ys == 0 ==> size v == 0)
        && (size xs > 0 && size ys > 0 ==> size v == size xs)} @-}
zipOrNull       :: [a] -> [b] -> [(a, b)]
zipOrNull [] _  = []
zipOrNull _ []  = []
...
\begin{code}
-- signature added
{-@ drop  :: n:Nat -> xs:ListGE a n -> {v:List a | size v == size xs - n} @-}
drop 0 xs     = xs
...
\begin{code}
-- signature added
{-@ take :: n:Nat -> xs:List a 
    -> {v:List a | size v == if size xs >= n then n else size xs} @-}
take 0 _       = []
take _ []      = []
take n (x:xs)  = x : take (n-1) xs
...
quickSort (x:xs) = join x small big
  where (small,big)  = partition (<=x) xs

{-@ join :: a -> xs:List a -> ys:List a 
              -> {v:List a | size v == size xs + size ys + 1} @-}
join z []     ys = z : ys 
...
\begin{code}
-- signature added
{-@ vecFromList :: x:List a -> VectorN a (size x) @-}
...
\begin{code}
{-@ as_regular_list ::  v:Vector a -> {r:[a] | (size r) == (vDim v)} @-}
as_regular_list :: Vector a -> [a]
as_regular_list (V m xs)  = xs

{-@ append_list :: f:[a] -> s:[a] -> ListN a {(size f) + (size s)}@-}
append_list [] s =  s
append_list (x:xs) s = x:append_list xs s
...
{-@ flatten :: n:Nat
  ...
  @-}
flatten 0 _ (V 0 [])      = (V 0 [])
flatten n m (V m' (x:xs)) = vecFromList (append_list 
                              (as_regular_list x) 
                              (as_regular_list (flatten (n-1) m (V (m'-1) xs)))) 
\end{code}
...
Solución dada por Hoang Nguyen, que tipa bien

{-@ convertList :: i:Nat -> {xss:List (List a) | notEmpty xss} -> Maybe (ListX (ListN a i) xss) @-}
convertList :: Int -> [[a]] -> Maybe [[a]]
convertList _ [] = die "May the odds be ever in our favor!"
convertList i [xs] = if i == size xs then Just [xs] else Nothing
convertList i (xs:xss) = if i == size xs then fmap (xs:) (convertList i xss) else Nothing
...
matFromList xss@(xs:_)
  ...
  where
    ...
    c            = size xs
    ok           = r > 0 && c > 0
    xss'         = convertList c xss
    vs           = fmap vecFromList (fmap (map vecFromList) xss')
-}

\begin{code}
-- copied this code form lists.hs
{-@ type NEList a = {v:[a] | notEmpty v} @-}
-- added this signature
{-@  matFromList :: x:NEList {v:[a] | notEmpty v && size v== size (head x)} 
     ->  Maybe (MatrixN a (size x) (size (head x))) @-}
matFromList      :: [[a]] -> Maybe (Matrix a)
matFromList []   = Nothing
matFromList xss@(xs:_)
  | ok           = Just (M r c vs)
  ...
  where
    ...
    c            = size xs
    ok           = sizesOK c xss
    vs           = vecsFromLists r c xss

    {-@ sizesOK:: m:Nat -> yss:NEList [a] ->  {v:Bool | v => size(head yss) == m} @-}
    sizesOK m [xs]     = size xs == m
    sizesOK m (xs:xss) = size xs == m && sizesOK m xss

    {-@ vecsFromLists :: m:Nat -> c:Nat -> ListN (ListN a c) m -> VectorN (VectorN a c) m @-}
    vecsFromLists :: Int -> Int -> [[a]] -> Vector (Vector a)
    vecsFromLists m c []       = vEmp
    vecsFromLists m c (ys:yss) = vCons (vecFromList ys) (vecsFromLists (m-1) c yss)
\end{code}
...
-- added the code below
txgo 0 _ _    = vEmp
txgo c r rows = vCons row1 $ txgo (c-1) r rest
   where row1   = V r xs
         V _ xs = for rows vHd
         rest   = for rows vTl
\end{code}
